/**********************************************************
 * @file    ADC.h
 * @brief   ADC Driver Header File (AUTOSAR Standard)
 * @details File này định nghĩa các Type chuẩn AUTOSAR cho ADC Driver,
...
#include "stm32f10x_adc.h"    /* Thư viện chuẩn của STM32F103 */
#include "stm32f10x_dma.h"
#include "misc.h"


/**
 * @typedef     Void Pointer Call back interrupt function
 * @brief       Void Pointer
 */
...
/**
 * @typedef     Configuration of a single ADC channel in a group, don't have in AUTOSAR
 * @brief       This structure holds the basic configuration for each ADC channel used within a group,
 *              including channel number, sampling time, and its position (rank) in the conversion sequence.
 */
...
/**
 * @typedef     Configuration structure for an ADC conversion group.
 * @brief       Defines how a group of ADC channels will be converted. Each group may contain
 * one or more channels with its own settings, trigger source, conversion mode, and ADC instance
 */
typedef struct {
    ...
    Adc_NotificationCbType IntNotificationCb;
    DMA_Channel_TypeDef*   DMA_Channel;
    uint8 Adc_StreamEnableType;             /* DMA có được bật hay không*/
    uint8 StreamBufferType;                 /* Kích thước bộ đệm*/
    Adc_StreamBufferModeType Adc_StreamBufferMode;  /* Mode Circular hay Linear*/
    Adc_NotificationCbType DMAIntNotificationCb;  /* Call back DMAk khi TC*/
} Adc_GroupDefType;

/**
 * @typedef     Global configuration structure for the ADC driver.
 * @brief       This structure aggregates all groups and optional initialization callback.
 *              It is passed to `Adc_Init()` to initialize the ADC driver with static configuration.
 */
typedef struct {
    uint8 NumGroups;
